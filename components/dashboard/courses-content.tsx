  ...
  return (
    <div ref={ref} className="relative">
      <button
        type="button"
        onClick={() => setOpen(v => !v)}
        className="w-full h-11 px-3 flex items-center justify-between rounded-md border border-white/10 text-sm text-white/80 hover:border-white/20 transition-colors"
      >
        <span>{value}</span>
        <ChevronDown
      ...
      {open && (
        <div className="absolute z-20 mt-1 w-full rounded-md border border-white/10 bg-black shadow-xl overflow-hidden">
          {options.map(opt => (
            <button
              key={opt}
              type="button"
              ...
              className={`w-full text-left px-3 py-2 text-sm transition-colors
                ${opt === value ? "bg-white/10 text-white" : "text-white/70 hover:bg-white/5"}
              `}
            >
              {opt}
            </button>
...
function CourseCard({
  ...
  if (viewMode === "list") {
    return (
      <Link
        href={link}
        className="group border border-white/10 rounded-xl bg-black hover:border-white/20 transition-colors overflow-hidden flex flex-col sm:flex-row"
      >
