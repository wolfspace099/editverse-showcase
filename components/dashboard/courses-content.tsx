    ...
    <div className="px-4 sm:px-6 lg:px-8">
      <div className="max-w-[1440px] mx-auto">
          ...
          <div className="mb-12 lg:mb-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 lg:gap-6">
              {continueLearning.slice(0, 3).map(progress => (
                <ContinueLearningCard key={progress.course_id} course={progress.courses} progress={progress.progress_percentage} lastAccessed={progress.last_accessed_at} />
              ))}
            </div>
          </div>
        )}
        ...
        <div className="mb-8 lg:mb-10">
          ...
          <p className="text-white/60 text-sm lg:text-base">Master video editing with our comprehensive course library</p>
        </div>
        ...
        <div className="mb-8 space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <Input type="text" placeholder="Search courses..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-11 pr-4 h-11 bg-transparent border border-white/10 rounded-md text-sm text-white placeholder:text-white/40 focus:border-white/30 focus:ring-0 transition-colors hover:border-white/20" />
            </div>

            <div className="flex items-center gap-3">
              <Button variant="outline" size="sm" onClick={() => setShowFilters(!showFilters)} className="sm:hidden flex-1 h-11 border-white/10 hover:bg-white/5">
                <Filter className="mr-2 h-4 w-4" /> Filters
              </Button>

              <div className="flex items-center border border-white/10 rounded-md">
                <button onClick={() => setViewMode("grid")} className={`p-2 ${viewMode === "grid" ? "bg-white/10" : "hover:bg-white/5"} transition-colors`}><Grid3x3 className="h-4 w-4 text-white/60" /></button>
                <button onClick={() => setViewMode("list")} className={`p-2 ${viewMode === "list" ? "bg-white/10" : "hover:bg-white/5"} transition-colors border-l border-white/10`}><List className="h-4 w-4 text-white/60" /></button>
              </div>
            </div>
          </div>
          ...
          <div className={`space-y-4 ${showFilters ? 'block' : 'hidden sm:block'}`}>
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {categories.map(cat => (
                  <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-4 py-2 rounded-full text-sm transition ${selectedCategory === cat ? "bg-white text-black" : "bg-white/5 text-white/70 hover:bg-white/10 border border-white/10"}`}>{cat}</button>
                ))}
              </div>
            </div>
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {difficulties.map(diff => (
                  <button key={diff} onClick={() => setSelectedDifficulty(diff)} className={`px-4 py-2 rounded-full text-sm transition ${selectedDifficulty === diff ? "bg-white text-black" : "bg-white/5 text-white/70 hover:bg-white/10 border border-white/10"}`}>{diff}</button>
                ))}
              </div>
            </div>
          </div>
        ...
        {/* Courses List */}
        {loading ? <div className="text-center py-20"><div className="inline-block h-8 w-8 animate-spin rounded-full border-4 border-solid border-white/20 border-r-white"></div></div> :
          courses.length === 0 ? (
            <div className="text-center py-20">
              <p className="text-white/60 mb-4">No courses found</p>
              <Button onClick={() => { setSearchQuery(""); setSelectedCategory("All"); setSelectedDifficulty("All") }} variant="outline" className="border-white/10 hover:bg-white/5">Clear filters</Button>
            </div>
          ) : (
            <div className={viewMode === "grid" ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 lg:gap-6" : "grid grid-cols-1 gap-4 lg:gap-5"}>
  ...
  return (
    <Link href={`/dashboard/?page=courses&course=${course.id}`} className="group border border-white/10 rounded-xl bg-black hover:border-white/20 transition-colors overflow-hidden">
      ...
      <div className="p-4 lg:p-5">
        <p className="text-xs text-white/50 mb-1">Last accessed {timeAgo}</p>
        <h3 className="text-base font-medium mb-3">{course.title}</h3>
